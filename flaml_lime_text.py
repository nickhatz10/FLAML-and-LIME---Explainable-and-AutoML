...
# function to get tf-idf of the tweets
def get_feature_vector(train_fit):
    vector = TfidfVectorizer(sublinear_tf=False)
...
tf_vector = get_feature_vector(tweets)

X_train_vec = tf_vector.transform(X_train)
y_train_vec = y_train.astype('int')
X_test_vec = tf_vector.transform(X_test)
y_test_vec = y_test.astype('int')
...
# gives KeyError when trying to pass through the y_train labels to this fit function
automl.fit(X_train = X_train_vec, y_train = y_train_vec, **automl_settings)

prediction = automl.predict(X_test)

# regular ski-kit learn models
...
y_predict_nb = NB_model.predict(X_test_vec)
print(accuracy_score(y_test_vec, y_predict_nb))

# Training Logistic Regression model
LR_model = LogisticRegression(solver='lbfgs')
LR_model.fit(X_train_vec, y_train_vec)
y_predict_lr = LR_model.predict(X_test_vec)
print(accuracy_score(y_test_vec, y_predict_lr))

# makes pipeline of ml classifier and feature vector of tweets
from sklearn.pipeline import make_pipeline
pipeline = make_pipeline(tf_vector, LR_model)
...
class flaml_lime_text_classification:
  ...
  def __init__(self, ml_pipeline, raw_test_data):
    self.ml_pipeline = ml_pipeline
    self.raw_test_data = raw_test_data

  # creates the LIME explainer
  def flaml_lime_text_explainer(self, class_names, **kwargs):
    import lime
    from lime import lime_text
    explainer = lime_text.LimeTextExplainer(class_names = class_names, **kwargs)
    return explainer
...
flaml_lime_text = flaml_lime_text_classification(raw_test_data = X_test, ml_pipeline = pipeline)

explainer = flaml_lime_text.flaml_lime_text_explainer(class_names = ['negative','positive'])

flaml_lime_text.flaml_lime_show(explainer, row_index = 1)